...
void write_block_bitmap(int fd) {
	...
	if (off == -1) {
  	...
  	if (write(fd, bbmap, sizeof(bbmap)) != sizeof(bbmap)) {
    	errno_exit("write");
  	}
}
...
void write_inode_bitmap(int fd) {
	...
	if (off == -1) {
  	...
  	if (write(fd, ibmap, sizeof(ibmap)) != sizeof(ibmap)) {
    	errno_exit("write");
  	}
}
...
void write_inode_table(int fd) {
	...
	write_inode(fd, EXT2_ROOT_INO, &root_inode);
	   //regular file called hello world
	   //simlink called hello that points to hellow world
	...
	memcpy((char *)hello_inode.i_block, "hello-world", sizeof(hello_inode.i_block));
...
void write_root_dir_block(int fd) {
	...
	struct ext2_dir_entry file_entry = {0};
	dir_entry_set(file_entry, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(file_entry, fd);

	bytes_remaining -= file_entry.rec_len;
...
void write_hello_world_file_block(int fd) {
	...
	ssize_t len = strlen(hello_world);
